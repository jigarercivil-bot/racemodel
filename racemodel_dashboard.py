...
data = q(conn, f"""
    WITH steam_cte AS (
        SELECT horse, course,
            ...
            LAST(current_odds ORDER BY captured_at) as latest_odds,
            MIN(current_odds) as lowest_odds,
            ROUND((MIN(morning_odds) - LAST(current_odds ORDER BY captured_at)) / NULLIF(MIN(morning_odds),0) * 100, 1) as steam_pct
        FROM steam_history
...
# ── Core Score Combination query ───────────────────────────────────────────────
core_combo = q(conn, f"""
    SELECT COUNT(*) as bets,
        SUM(CASE WHEN dc.place_result=1 THEN 1 ELSE 0 END) as placed,
        ROUND(100.0*SUM(CASE WHEN dc.place_result=1 THEN 1 ELSE 0 END)/NULLIF(COUNT(*),0),1) as place_pct,
        ROUND(AVG(sh.morning_odds),2) as avg_odds,
        ROUND(AVG(sh.steam_pct),1) as avg_move,
        ROUND(AVG(dc.score - COALESCE(dc.s4_bsp_ratio,0)),2) as avg_core
    FROM daily_candidates dc
    INNER JOIN (
        SELECT horse, course, race_date,
            MIN(morning_odds) as morning_odds,
            ROUND((MIN(morning_odds)-LAST(current_odds ORDER BY captured_at))/NULLIF(MIN(morning_odds),0)*100,1) as steam_pct
        FROM steam_history
        GROUP BY horse, course, race_date
    ) sh ON LOWER(TRIM(dc.horse))=LOWER(TRIM(sh.horse))
        AND LOWER(TRIM(dc.course))=LOWER(TRIM(sh.course))
        AND dc.race_date=sh.race_date
    WHERE dc.race_date = {date_sql}
        AND (dc.score - COALESCE(dc.s4_bsp_ratio,0)) >= 5.5
        AND sh.morning_odds BETWEEN 3 AND 8
        AND sh.steam_pct >= 10""")
if not data.empty:
...
with right_col:
    ...
    st.markdown("<div class='section-card'>", unsafe_allow_html=True)
    st.markdown("<div class='section-hdr'>🎯 CORE SCORE COMBINATION</div>", unsafe_allow_html=True)
    st.markdown("""
    <div style='display:flex;gap:4px;flex-wrap:wrap;margin-bottom:8px'>
      <span style='background:#15803d;color:#fff;padding:2px 7px;border-radius:4px;font-size:9px;font-weight:700'>Core ≥5.5 (Score−S4)</span>
      <span style='background:#15803d;color:#fff;padding:2px 7px;border-radius:4px;font-size:9px;font-weight:700'>Odds $3–$8</span>
      <span style='background:#15803d;color:#fff;padding:2px 7px;border-radius:4px;font-size:9px;font-weight:700'>Steam ≥10%</span>
    </div>""", unsafe_allow_html=True)

    if not core_combo.empty and core_combo.iloc[0]["bets"] and int(core_combo.iloc[0]["bets"]) > 0:
        cc = core_combo.iloc[0]
        c1, c2, c3, c4, c5 = st.columns(5)
        for col, label, val, color in [
            (c1, "Bets",     int(cc["bets"]) if pd.notna(cc["bets"]) else 0,                                    "#111827"),
            (c2, "Place %",  f"{cc['place_pct']}%" if pd.notna(cc['place_pct']) else "—",                       "#15803d"),
            (c3, "Avg Odds", f"${float(cc['avg_odds']):.2f}" if pd.notna(cc['avg_odds']) else "—",              "#d97706"),
            (c4, "Avg Move", f"+{cc['avg_move']}%" if pd.notna(cc['avg_move']) else "—",                        "#0777b3"),
            (c5, "Avg Core", f"{float(cc['avg_core']):.2f}" if pd.notna(cc['avg_core']) else "—",               "#7c3aed"),
        ]:
            col.markdown(f"""
            <div style='background:#f0fdf4;border-radius:6px;padding:4px 2px;text-align:center;
                 border:1px solid #d1fae5;margin-bottom:4px'>
              <div style='font-size:11px;font-weight:800;color:{color};white-space:nowrap'>{val}</div>
              <div style='font-size:7px;color:#6b7280;margin-top:1px'>{label}</div>
            </div>""", unsafe_allow_html=True)
    else:
        st.markdown("<div style='font-size:10px;color:#6b7280'>No data for selected date</div>", unsafe_allow_html=True)

    st.markdown("<div style='font-size:8px;color:#9ca3af;margin-top:4px'>Selected date · Core = Score − S4</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
