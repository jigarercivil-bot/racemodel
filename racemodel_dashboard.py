...
with right_col:
    ...
    if not core_combo.empty and core_combo.iloc[0]["bets"] and int(core_combo.iloc[0]["bets"]) > 0:
        ...
        for col, label, val, color in [
            (c1, "Bets",     int(cc["bets"]),                    "#111827"),
            (c2, "Place %",  f"{cc['place_pct']}%",              "#15803d"),
            (c3, "Avg Odds", f"${float(cc['avg_odds']):.2f}",    "#d97706"),
            (c4, "Avg Move", f"+{cc['avg_move']}%",              "#0777b3"),
            (c5, "Avg Core", f"{float(cc['avg_core']):.2f}",     "#7c3aed"),
        ]:
            col.markdown(f"""
            <div style='background:#f0fdf4;border-radius:6px;padding:6px 4px;text-align:center;
                 border:1px solid #d1fae5;margin-bottom:4px'>
              <div style='font-size:13px;font-weight:800;color:{color}'>{val}</div>
              <div style='font-size:8px;color:#6b7280;margin-top:2px'>{label}</div>
            </div>""", unsafe_allow_html=True)
    else:
